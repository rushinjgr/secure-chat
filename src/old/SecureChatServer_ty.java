package old;/* @Programmer: John Ramirez
...
public class SecureChatServer_ty {
    ...
    public SecureChatServer_ty(int MaxU) throws IOException
    {
        MaxUsers = MaxU;
                        ...
                        
    {
        for (int i = 0; i < numUsers; i++)
        {
			...
			SymCipher_ty currCipher = threads[i].getCipher();
			...
			if (numUsers < MaxUsers)
			{
				try
				{
					...
					if (encType.equals("Add"))
					{
						...
						cipher = new Add128_ty(byteKey);
					}
					else
					{
						System.out.println("Chosen Cipher is Substitute");
						cipher = new Substitute_ty(byteKey);
					}
    ...
    private class UserThread extends Thread
    {
         ...
         private SymCipher_ty myCipher;
         private int myId;
         private String myName;

         private UserThread(Socket newSocket, int id, String newName,
                            ObjectInputStream newReader,
							ObjectOutputStream newWriter, SymCipher_ty c) throws IOException
         {
              mySocket = newSocket;
              myId = id;
              myName = newName;
              ...
              while (ok)
              {
                    ...
                    try {
						 ...
						 if (newBytes == null || newMsg.equals("CLIENT CLOSING"))
                             	...
                             	SecureChatServer_ty.this.SendMsg(newMsg);
                    }
                    catch (Exception e)
                    {
    ...
    {
         SecureChatServer_ty Secure = new SecureChatServer_ty(30);
    }
}
